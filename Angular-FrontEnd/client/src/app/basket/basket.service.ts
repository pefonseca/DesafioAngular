...
export class BasketService {
  ...
  addItemToBasket(item: IProduto, quantidade = 1) {
    const itemToAdd: IBasketItem = this.mapProdutoItemToBasketItem(item, quantidade);
  ...
  incrementItemQuantity(item: IBasketItem) {
    ...
    basket.items[foundItemIndex].quantidade++;
    this.setBasket(basket);
  }
  ...
  decrementItemQuantity(item: IBasketItem) {
    const basket = this.getCurrentBasketValue();
    const foundItemIndex = basket.items.findIndex(x => x.id === item.id);
    if (basket.items[foundItemIndex].quantidade > 1) {
      basket.items[foundItemIndex].quantidade--;
      this.setBasket(basket);
    } else {
  ...
  private mapProdutoItemToBasketItem(item: IProduto, quantidade: number): IBasketItem {
    return {
      id: item.id,
